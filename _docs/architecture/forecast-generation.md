...
1. **Fetch** — `src/integrations/DMI.php` pulls forecast grids from the Danish Meteorological Institute's govcloud API (HARMONIE_DINI_SF model) and station observations (wind dir/speed/gusts, precipitation). `src/integrations/openMeteo.php` separately serves point forecasts via Open-Meteo.com. Raw fetched data is written under `SBconfig::DIR_FORECASTDATA`.
